...
public class Base64 {
	...
	private static int decode4to3(byte[] source, int srcOffset, byte[] destination, int destOffset, int options) {
		...
		// Example: DkLE
			...
			try {
				...
				Main.debug.println("EXCEPTION (static), Base64.java):");
				Main.debug.println("\t" + source[srcOffset] + ": " + (DECODABET[source[srcOffset]]));
				Main.debug.println("\t" + source[srcOffset + 1] + ": " + (DECODABET[source[srcOffset + 1]]));
				Main.debug.println("\t" + source[srcOffset + 2] + ": " + (DECODABET[source[srcOffset + 2]]));
				Main.debug.println("\t" + source[srcOffset + 3] + ": " + (DECODABET[source[srcOffset + 3]]));
				return -1;
			} // end catch
		}
	} // end decodeToBytes
	...
	public static byte[] decode(byte[] source, int off, int len, int options) {
		...
		for (i = off; i < off + len; i++) {
			                                  ...
			                                  // better
			{
				...
				Main.debug.println("ERROR (static): Bad Base64 input character at " + i + ": " + source[i]
				        + "(decimal)");
				return null;
			} // end else:
	...
	public static byte[] decodeFromFile(String filename) {
		...
		try {
			...
			if (file.length() > Integer.MAX_VALUE) {
				Main.debug.println("ERROR (static): File is too big for this convenience method (" + file.length()
				        + " bytes).");
				return null;
			...
			Main.debug.println("EXCEPTION (static): Error decoding from file " + filename);
		} // end catch: IOException
		finally {
			try {
				bis.close();
	...
	public static String encodeFromFile(String filename) {
		Main.debug.println("INVOKED (static) filius.rahmenprogramm.Base64, encodeFromFile(" + filename + ")");
		...
		try {
			...
			// Main.debug.println("DEBUG, Base64, encodeFromFile: bis=" +
			// (bis==null ? "<null>" : bis) );
			...
			// Main.debug.println("DEBUG, Base64, encodeFromFile: length=" +
			// length );

			// Save in a variable to return
			encodedData = new String(buffer, 0, length, Base64.PREFERRED_ENCODING);

			// Main.debug.println("DEBUG, Base64, encodeFromFile: encodedDate="
			// + encodedData );

		} // end try
		catch (java.io.IOException e) {
			Main.debug.println("ERROR (static): Error encoding from file " + filename);
		} // end catch: IOException
		finally {
			try {
				bis.close();
